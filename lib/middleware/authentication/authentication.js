...
function authentication () {
  ...
  // INCLUDES EXTRA_PASSWORDS PENSION CREDIT HACKS
  const extraPasswords = process.env.EXTRA_PASSWORDS
    ? process.env.EXTRA_PASSWORDS.split(',').map((pass) => encryptPassword(pass))
    : []
  const passwords = [encryptPassword(process.env.PASSWORD), ...extraPasswords]

  return function authentication (req, res, next) {
    if (allowedPathsWhenUnauthenticated.includes(req.path)) {
      next()
    } else if (isAuthenticated(passwords, req)) {
      next()
    } else {
...
function isAuthenticated (encryptedPasswords, req) {
  return encryptedPasswords.includes(req.cookies.authentication)
}
